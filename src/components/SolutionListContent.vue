  ...
  <div data-v-8f4c44c2 class="newsNew">
      <div data-v-8f4c44c2 class="news-center-filter">
          <div data-v-8f4c44c2 class="z-segment is-round" style="font-weight: bold;">
            <div class="z-segment-item first">题解</div>
          </div>
                  ...
                  <div data-v-5773a1c6 element-loading-background="rgba(0, 0, 0, 0)" class="child-news-main-content" style="width:100%" v-loading="loading">
                    <el-card class="box-card" style="margin-bottom:10px" v-for="(item,index) in slogList" :key="index">
                        <div slot="header" class="headerter">
                            <div class="header-left">
                                <div style="float:left">
                                    ...
                                    <span>{{item.correspondingId}}</span>&nbsp;
                                    <span>{{item.correspondingTitle}}</span>&nbsp;
                                    <a :href="item.correspondingLink" style="text-decoration:none;color:#29E6D3" v-show="isShow(item.correspondingLink)" id="href">题目链接</a>
                                </div>
                        ...
                        <el-row :gutter="20">
                            ...
                            <el-col :span="5">
                                <span><strong>{{item.authorId}}</strong></span>
                                <br>
                        ...
                        <div style="padding-left:70px">
                            <div class="file-content-field">
                                <div style="white-space: pre-wrap; word-break: break-all; padding-right: 25px; margin-top: 10px;max-height: 70px;">
                                {{item.content}}
                                </div>
                            </div>
                        </div>
...
export default {
    ...
    methods:{
        ...
        handleCurrentChange(val) {
            ...
            correspondingId:window.sessionStorage.getItem('problemId'),
            schoolId:window.sessionStorage.getItem('schoolId')
            })
            this.$axios.post('http://127.0.0.1:9090/slog/slogListByCorrespondingId',data).then((response)=>{
                if(response.data.code == '201'){
