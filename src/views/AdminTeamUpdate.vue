...
export default {
  ...
  mounted () {
    if (this.$store.state.tab.isUpdate) {
      ...
      this.operateForm.name = this.$store.state.tab.tempObj.name
      this.operateForm.intro = this.$store.state.tab.tempObj.intro
    }
  },
  methods: {
    confirm (form) {
      if (this.formTitle === '修改队伍') {
        api.teamUpdate((form)).then(res => {
